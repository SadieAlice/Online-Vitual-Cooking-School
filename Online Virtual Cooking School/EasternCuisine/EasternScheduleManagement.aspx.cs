...
{
    public partial class EasternScheduleManagement : System.Web.UI.Page
    {
        ...
        protected void ButSaveSchedule_Click(object sender, EventArgs e)
        {
            ...
            string sqlquery = "Insert into [dbo].[EasternCuisineSchedule] (Sday,Stime,Elevel,Elocation,ERoom,Ecapacity) values (@Sday,@Stime, @Elevel,@Elocation, @ERoom,@Ecapacity)";
        ...
        protected void GdSchedule_RowUpdating(object sender, GridViewUpdateEventArgs e)
        {
            ...
            string mainconn = ConfigurationManager.ConnectionStrings["conn"].ConnectionString;
            SqlConnection sqlconn = new SqlConnection(mainconn);
            string sqlquery = "update EasternCuisineSchedule set Sday=@Sday, Stime=@Stime, Elevel=@Elevel,Elocation=@Elocation,ERoom=@ERoom,Ecapacity=@Ecapacity where Eid=@Eid";
            sqlconn.Open();
            SqlCommand sqlcomm = new SqlCommand(sqlquery, sqlconn);
        ...
        protected void GdSchedule_RowDeleting(object sender, GridViewDeleteEventArgs e)
        {
            ...
            string mainconn = ConfigurationManager.ConnectionStrings["conn"].ConnectionString;
            SqlConnection sqlconn = new SqlConnection(mainconn);
            string sqlquery = "delete from EasternCuisineSchedule where Eid=@Eid";
            sqlconn.Open();
            SqlCommand sqlcomm = new SqlCommand(sqlquery, sqlconn);
            sqlcomm.Parameters.AddWithValue("@Eid", Eid);
            sqlcomm.ExecuteNonQuery();
